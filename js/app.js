...
const SASP = (() => {
  ...
  function _card(p) {
    ...
    const sentence = p.isLife
      ? `<span class="card-life">DOŽIVOTÍ</span>`
      : (p.jail > 0 ? `<span class="card-jail">${p.jail} LET</span>` : '');

    const fine = p.fine > 0
      ? `<span class="card-fine">${p.fine.toLocaleString('cs-CZ')} $</span>`
      : '';
    ...
    return `
      <div class="protocol-card">
        ...
        <div class="card-sentence">${sentence}${fine}</div>
  ...
  const Clip = {
    copy() {
      ...
      if (hasZP)  out += ' [ZP]  ODEBRAT ZBROJNÍ PRŬkAZ\n';
      if (hasRP)  out += ' [RP]  ODEBRAT ŘIDIČSKÝ PRŬkAZ\n';
      out += '════════════════════════════════════════';

      if (navigator.clipboard) {
        navigator.clipboard.writeText(out)
          .then(() => UI.toast('✓ Protokol zkopírován!'))
          .catch(() => Clip._fallback(out));
      } else {
        Clip._fallback(out);
      }
    },
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('copyBtn')
    .addEventListener('click', () => SASP.copyProtocol());
